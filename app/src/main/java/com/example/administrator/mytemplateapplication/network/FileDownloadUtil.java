...
public class FileDownloadUtil {
    ...
    public static boolean writeResponseBodyToDisk(ResponseBody body, String filename) {
        ...
        // 目录不存在  则创建
        File dir = new File(Constant.ROOT_PATH);
        if (!dir.exists()) {
            dir.mkdirs();
        }
        try {
            File         futureStudioIconFile = new File(Constant.ROOT_PATH + File.separator + filename);
    ...
    public static boolean isExists(String filename) {
        try {
            File file = new File(Constant.ROOT_PATH + File.separator + filename);
            if (!file.exists()) {
                return false;
            }
        } catch (Exception e) {

...
public class MainActivity extends LifecycleActivity implements
    ...
    private MainActivtiyViewModel mViewModel;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mViewModel = ViewModelProviders.of(this, factory).get(MainActivtiyViewModel.class);
        mViewModel.getWeather().observe(this, weatherEntry -> {
            mForecastAdapter.swapForecast(weatherEntry);
            if (mPosition == RecyclerView.NO_POSITION) mPosition = 0;
            mRecyclerView.smoothScrollToPosition(mPosition);

            // Show the weather list or the loading screen based on whether the forecast data exists
            // and is loaded
            if (weatherEntry != null && weatherEntry.size() != 0) showWeatherDataView();
            else showLoading();
        });
    }
